...
void Editor::HandleClick() {
	//STUMOD
	// if we are in performance mode, the clock should send us back to the load page
  if (editor_mode_ == EDITOR_MODE_PERFORMANCE) {
    editor_mode_ == EDITOR_MODE_PATCH;
    JumpToPageGroup(GROUP_LOAD_SAVE); }

    // end stu mod
  else if (display_mode_ == DISPLAY_MODE_OVERVIEW) {
...
void Editor::HandleLoadSaveClick() {
  if (action_ == ACTION_LOAD) {
    //stuMOD
    // I don't need the compare function, which was the next 2 lines
    //action_ = ACTION_COMPARE;
    //RestoreEditBuffer();
    //
    // instead, we now jump to the performance page
    editor_mode_ = EDITOR_MODE_PERFORMANCE;
    JumpToPageGroup(GROUP_PERFORMANCE);
...
uint8_t Editor::KnobIndexToParameterId(uint8_t knob_index) {
  if (current_page_ == PAGE_PERFORMANCE || current_page_ == PAGE_LOAD_SAVE) {
    ...
    //STUMOD
    //return Octave/Semitone Knob (parameter 44) for First Knob in all cases
    if (knob_index == 0) {
		return 44;
  } else {
